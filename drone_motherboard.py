...
from acroname_usb_switch_plug import AcronameUsbSwitchPlug
from dfu_programmer_plug import DfuProgrammerPlug
from drone_com_plug import DroneComPlug
from lsusb_plug import LsusbPlug
from psu_control_plug import PsuControlPlug
...
@plug(lsusb=LsusbPlug, usb_switch=AcronameUsbSwitchPlug, power_supply=PsuControlPlug)
@htf.PhaseOptions(repeat_limit=5, timeout_s=10)
def check_mb_alive(lsusb, usb_switch, power_supply):
    usb_switch.turn_on(port=1)  # Turn on USB first
    power_supply.turn_on()  # Then turn power on to enter DFU mode

    if CONF.mb_usb_id in lsusb.list():
...
def set_drone_id(test, drone_com):
    test.measurements.drone_production_id=drone_com.set_drone_id(test.test_record.dut_id)
...
def check_pressure_sensor(test, drone_com):
    test.measurements.pressure = drone_com.get_pressure()


## Teardown
@plug(usb_switch=AcronameUsbSwitchPlug, power_supply=PsuControlPlug)
def teardown(usb_switch, power_supply):
    usb_switch.turn_off(port=1)
